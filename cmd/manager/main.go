...
func main() {
	...
	err = leader.Become(ctx, "machine-maintenance-operator-lock")
	if err != nil {
		log.Error(err, "")
		os.Exit(1)
	}
	...
	// Define stopCh which we'll use to notify the maintenancewatcher (and any other routine)
	// to stop work. This channel can also be used to signal routines to complete any cleanup
	// work
	//	stopCh := signals.SetupSignalHandler()

	// Initialize the MaintenanceWatcher
	maintenancewatcher.Initialize(mgr.GetClient(), maintenanceWatcherScanInterval)

	// Start the maintenanceWatcher
	//go maintenancewatcher.MaintenanceWatcher.Start(log, stopCh)
	maintenancewatcher.MaintenanceWatcher.Start()
